...
rubbish_summary_grouped <- function(dataframe, Category, dec_point){
  ...
  numerics$Category <- Category
  
  # Get summary for numeric data. 
  # Gather. 
  numerics_out <- tidyr::gather(numerics, "Variable", "Value", -Category)
  # Grouping variables. 
  numerics_out <- dplyr::group_by(numerics_out, Category, Variable)
  # Summarise. 
